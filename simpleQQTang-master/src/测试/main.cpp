...
unsigned char* load_cursor_image(const char* filename, int* width, int* height) {
    unsigned char* img = SOIL_load_image(filename, width, height, 0, SOIL_LOAD_RGBA);
    if (img) {
        std::cout << "[DEBUG] Loaded cursor image from: " << filename << std::endl;
        return img;
    }
    // Try parent directory
    std::string parent_path = std::string("../") + filename;
    img = SOIL_load_image(parent_path.c_str(), width, height, 0, SOIL_LOAD_RGBA);
    if (img) {
        std::cout << "[DEBUG] Loaded cursor image from: " << parent_path << std::endl;
        return img;
    }
    // Try absolute path (if known)
    std::string abs_path = std::string("/home/jiaqig/Downloads/copilot_play/simpleQQTang-master/src/æµè¯/") + filename;
    img = SOIL_load_image(abs_path.c_str(), width, height, 0, SOIL_LOAD_RGBA);
    if (img) {
        std::cout << "[DEBUG] Loaded cursor image from: " << abs_path << std::endl;
        return img;
    }
    std::cout << "[DEBUG] Failed to load cursor image: " << filename << std::endl;
    return nullptr;
}
...
void mouse_position_callback(GLFWwindow* window, double xpos, double ypos)
{
    int width, height;
    int width2, height2;
    unsigned char* image1 = load_cursor_image("手套鼠标.png", &width, &height);
    unsigned char* image2 = load_cursor_image("手套鼠标闭合.png", &width2, &height2);
    GLFWimage Image;
    Image.width = width;
    Image.height = height;
    Image.pixels = image1;
    GLFWcursor* cursor = nullptr;
    if (image1) cursor = glfwCreateCursor(&Image, 0, 0);
    GLFWimage Image2;
    Image2.width = width2;
    Image2.height = height2;
    Image2.pixels = image2;
    GLFWcursor* cursor2 = nullptr;
    if (image2) cursor2 = glfwCreateCursor(&Image2, 0, 0);
    if (scene == 1) {
        if (!Mistakewindow) {
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    ...
    unsigned char* image1 = load_cursor_image("手套鼠标.png", &width, &height);
    unsigned char* image2 = load_cursor_image("手套鼠标闭合.png", &width2, &height2);
    unsigned char* image3 = load_cursor_image("手套鼠标按下.png", &width3, &height3);
    GLFWimage Image;
    Image.width = width;
    Image.height = height;
    Image.pixels = image1;
